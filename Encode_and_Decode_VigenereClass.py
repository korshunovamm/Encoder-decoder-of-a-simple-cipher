...
class EncodeVigenereClass(AbsCodeClass):
    ...
    поданного на вход через консоль.
    Typecode принимает значение '+1' в случае кодирования,
    '-1' в случае декодирования.
    ...
    def __init__(self, text, window, bg_color):
        ...
        txt.pack()
        ready = Button(self.window, text="Готово!",
                       command=clicked, bg=self.bg_color)
        ready.pack()
        ...
        if self.code_vig is not None:
            ...
            for char, i in zip(self.text, itertools.cycle(range(len_key))):
                if char.isupper():
                    ...
                    value = (ord(char)
                             + self.typecode*(ord(self.code_vig[i].upper())
                                              - self.bigstartletter)
                    ...
                    value = (ord(char)
                             + self.typecode*(ord(self.code_vig[i].lower())
...
class DecodeVigenereClass(EncodeVigenereClass):
    ...
    Typecode принимает значение '+1' в случае кодирования,
    '-1' в случае декодирования.
    Далее используется функция кодирования текста, которая
